...
bool is_non_terminal(string& str, map<string, Definition>& grammar){
  // tu araterminaluria mapshic agar meqneba
  return grammar.find(str) != grammar.end();
}

void rame(map<string, Definition>& grammar, string& res, string non_terminal){
  auto it = grammar[non_terminal].getRandomProduction();
  auto i = it.begin();

  for(; i != it.end(); i++){
    string str = *i;
    if(is_non_terminal(str, grammar)) rame(grammar, res, str);
    else res += (" " + str);
  }
}
