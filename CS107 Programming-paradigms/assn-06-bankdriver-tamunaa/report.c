 ...
 */
int
Report_Transfer(Bank *bank, int workerNum, AccountNumber accountNum, AccountAmount amount){
...
int Report_DoReport(Bank *bank, int workerNum){
  ...
  if(bank->has_to_finish != 0){
    sem_post(&(bank->check));
    sem_wait(&(bank->next_day_starter));
    sem_post(&(bank->tamuna));
  }else{
    bank->has_to_finish = bank->workers;

    Report *rpt = bank->report;
    assert(rpt); Y;

    if (rpt->numReports >= MAX_NUM_REPORTS) {
      for(int i = 0; i < bank->workers-1; i++){
        sem_post(&bank->next_day_starter);
      }
      for(int i = 0; i < bank->workers-1; i++){
        sem_wait(&bank->tamuna);
      }
      sem_post(&bank -> check);
      return -1;
    }
