...
(define (intersection-points ls)
    ...
    ((null? ls) '())
    (else 
      (append (apply append (map (lambda (elem) (intersect (car ls) elem)) (cdr ls)))
...
(define (helper_for_product list p) 
  (map (lambda (elem) (dist elem p)) list)
)

(define (positive x) (> x 0))

(define (filter_products list)
  (filter list positive?)
)

(define (distance-product p list)
  (apply *(filter_products (helper_for_product list p)))
)

(define (rate-points l)
  (map (lambda(elem)(list (distance-product elem l) elem)) l)
)

(define (is_less el1 el2) (< (car el1) (car el2)))

(define (sort-points ls)
  (quicksort ls is_less)
)

(define (clumped-points ls)
  (map cadr(prefix-of-list (sort-points (rate-points ls)) (quotient (length ls) 2)))
)

(define (sumX ls)
  (apply + (map (lambda (elem) (car elem)) ls))
)

(define (sumY ls)
  (apply + (map (lambda (elem) (cadr elem)) ls))
)

(define (average-point ls)
  (let ((p (list (/ (sumX ls)(length ls)) (/ (sumY ls)(length ls)))))
    (list (distance-product p ls) p))
)
...
(define (where-am-i lens st)
  (sort-points (map (lambda (elem) (best-estimate elem))(all-guesses lens st)))
)
