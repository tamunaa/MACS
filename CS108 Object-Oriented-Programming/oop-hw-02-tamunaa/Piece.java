...
public class Piece {
	...
	*/
	...
	private void setWidthAndHeight(){
		...
		for (TPoint p : this.body){
			curWidth = Math.max(p.x, curWidth);
			curHeight = Math.max(p.y, curHeight);
		}

		this.width = curWidth+1;
		this.height = curHeight+1;
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] rotatedPoints = Arrays.stream(this.body).
				map(curp -> new TPoint(getHeight() - 1 - curp.y, curp.x))
				.toArray(TPoint[]::new);
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while (true){
			Piece rotated = curPiece.computeNextRotation();
			if (rotated.equals(root))break;
			curPiece.next = rotated;
			curPiece = rotated;
		}
		curPiece.next = root;
